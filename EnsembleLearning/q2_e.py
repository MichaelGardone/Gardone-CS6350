...
def main():
    global FEATURES, T, DEBUG, M, NUM_OF_TREES, SUB_ATTRIB_COUNT

    print("=== Beginning Q2, part E tests! ===")
    ...
    for t in range(NUM_OF_TREES):
        ...
        rf = RandomForest.RandomForest("y", sample_count, SUB_ATTRIB_COUNT)
        rf.random_forest(subset_training, FEATURES, T)
        bag_o_rfs.append(rf)
        # For comparison, pick the first tree in each run to get 100 fully expanded trees
        bag_o_single_trees.append(rf.get_hypothesis(0))
    ...
    for i in range(testing_length):
        ...
        predictions = []
        for t in range(NUM_OF_TREES):
            predictions.append(bag_o_rfs[t].classify(te.iloc[i]))
    ...
    print("=== Finished Q2, part E tests! ===")
